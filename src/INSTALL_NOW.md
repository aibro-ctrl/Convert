# 🚀 Установка Backend - СЕЙЧАС

## ⚡ Быстрое решение вашей ошибки

Ошибка `PocketBase is not a constructor` возникла из-за неправильного импорта модуля.

### 🔧 Решение (ИСПРАВЛЕНО):

Я исправил все файлы. Теперь установите снова:

```bash
# 1. Перейдите в backend-setup
cd /var/www/Convert/src/backend-setup

# 2. Удалите старые зависимости
rm -rf node_modules package-lock.json

# 3. Установите зависимости
npm install pocketbase ioredis dotenv

# 4. Создайте коллекции
node create-collections.js

# 5. Проверьте подключение
node test-connection.js
```

**Готово!** Теперь всё должно работать.

---

## 📝 Что было исправлено

В файлах `create-collections.js` и `test-connection.js`:

**Было:**
```javascript
const PocketBase = require('pocketbase');
```

**Стало:**
```javascript
const PocketBase = require('pocketbase').default || require('pocketbase');
```

Это исправляет проблему с ESM/CommonJS модулями.

---

## 🎯 Полная последовательность установки

### 1. Установите зависимости:

```bash
cd /var/www/Convert/src/backend-setup
npm install pocketbase ioredis dotenv
```

### 2. Проверьте установку:

```bash
node -e "console.log(require('pocketbase'))"
# Должно показать: [Function: default]
```

### 3. Создайте коллекции:

```bash
node create-collections.js
```

### 4. Проверьте подключение:

```bash
node test-connection.js
```

---

## 🔍 Проверка перед установкой

Убедитесь что всё готово:

```bash
# PocketBase запущен?
curl http://localhost:8090/api/health
# Должен ответить: {"code":200,"message":"API is healthy.","data":{}}

# Redis работает?
redis-cli ping
# Должен ответить: PONG

# Node.js установлен?
node --version
# Должен показать: v20.x.x или выше
```

---

## ✅ Порядок действий ПРЯМО СЕЙЧАС

```bash
# ШАГ 1: Установка зависимостей
cd /var/www/Convert/src/backend-setup
npm install pocketbase ioredis dotenv

# ШАГ 2: Создание коллекций
node create-collections.js

# ШАГ 3: Тест подключения
node test-connection.js

# ШАГ 4: Создание .env (если еще нет)
cd ..
cat > .env << 'EOF'
VITE_BACKEND_TYPE=pocketbase
VITE_POCKETBASE_URL=http://localhost:8090
VITE_REDIS_HOST=localhost
VITE_REDIS_PORT=6379
VITE_REDIS_DB=0
VITE_E2EE_ENABLED=true
VITE_REALTIME_ENABLED=true
JWT_SECRET=$(openssl rand -base64 32 | tr -d "=+/" | cut -c1-32)
ENCRYPTION_KEY=$(openssl rand -base64 32 | tr -d "=+/" | cut -c1-32)
EOF

# ШАГ 5: Создайте администратора
echo "Откройте http://localhost:8090/_/ и создайте админа"

# ШАГ 6: Запустите приложение
npm run dev
```

---

## 🎉 После успешной установки

Вы увидите:

```
📦 Создание коллекций в PocketBase...
✓ Коллекция "users" создана
✓ Коллекция "rooms" создана
✓ Коллекция "messages" создана
✓ Коллекция "achievements" создана
✓ Коллекция "user_achievements" создана
✓ Коллекция "friend_requests" создана
✓ Коллекция "files" создана

🏆 Создание дефолтных достижений...
  ✓ Создано: 🎉 Первое сообщение
  ✓ Создано: 🌙 Полуночник
  ✓ Создано: 💬 Болтун
  ... и т.д.

✅ Создание коллекций завершено!
```

---

## 🐛 Если всё еще есть проблемы

### Проблема: node_modules не создается

```bash
cd /var/www/Convert/src/backend-setup
rm -rf node_modules package-lock.json
npm install
```

### Проблема: PocketBase не отвечает

```bash
# Проверьте процесс
ps aux | grep pocketbase

# Запустите вручную
cd /opt/pocketbase
./pocketbase serve --http=0.0.0.0:8090
```

### Проблема: Права доступа

```bash
# Дайте права на папку
sudo chown -R $USER:$USER /var/www/Convert
```

---

## 📞 Нужна помощь?

Отправьте вывод этих команд:

```bash
# Информация о системе
node --version
npm --version
redis-cli --version

# Статус сервисов
curl http://localhost:8090/api/health
redis-cli ping

# Структура проекта
ls -la /var/www/Convert/src/backend-setup/
```

---

**Версия:** 1.0.1 (исправлена ошибка с зависимостями)  
**Дата:** 01.12.2025  
**Статус:** ✅ Исправлено и готово к установке