# 🚀 Нативная установка Backend (БЕЗ Docker)

## ⚡ Быстрый старт

### Для Ubuntu с уже установленными PocketBase и Redis:

```bash
cd backend-setup
chmod +x setup.sh
./setup.sh
```

**Готово!** Скрипт всё настроит автоматически за ~5 минут.

---

## 📋 Что делает скрипт?

1. ✅ Проверяет PocketBase и Redis
2. ✅ Генерирует безопасные ключи
3. ✅ Создает .env конфигурацию
4. ✅ Запускает PocketBase (если не запущен)
5. ✅ Создает 7 коллекций в PocketBase
6. ✅ Добавляет 8 дефолтных достижений
7. ✅ Устанавливает NPM зависимости
8. ✅ Создает systemd сервисы
9. ✅ Настраивает Redis оптимизацию
10. ✅ Тестирует все подключения

---

## 🛠️ Что нужно ДО запуска?

### 1. Установите PocketBase (если еще нет):

```bash
# Скачайте
cd /tmp
wget https://github.com/pocketbase/pocketbase/releases/download/v0.22.0/pocketbase_0.22.0_linux_amd64.zip

# Распакуйте
sudo mkdir -p /opt/pocketbase
sudo unzip pocketbase_0.22.0_linux_amd64.zip -d /opt/pocketbase
sudo chmod +x /opt/pocketbase/pocketbase

# Проверьте
/opt/pocketbase/pocketbase --version
```

### 2. Установите Redis (если еще нет):

```bash
sudo apt update
sudo apt install redis-server -y

# Запустите
sudo systemctl enable redis-server
sudo systemctl start redis-server

# Проверьте
redis-cli ping
# Должен ответить: PONG
```

### 3. Установите Node.js 18+ (если еще нет):

```bash
curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -
sudo apt install -y nodejs

# Проверьте
node --version
npm --version
```

---

## 📁 Созданные файлы

```
backend-setup/
├── setup.sh               # Основной скрипт установки ⭐
├── create-collections.js  # Создание коллекций PocketBase
├── redis-cache.js         # Redis Cache Manager
├── test-connection.js     # Тест подключений
└── README.md             # Детальная документация
```

---

## 🔧 Созданные коллекции

После установки в PocketBase будут созданы:

1. **users** (auth) - пользователи + E2EE ключи
2. **rooms** - комнаты (публичные/приватные/DM)
3. **messages** - сообщения + реакции
4. **achievements** - достижения
5. **user_achievements** - прогресс достижений
6. **friend_requests** - запросы дружбы
7. **files** - загруженные файлы

---

## 🎯 После установки

### Создайте первого администратора:

1. Откройте: http://localhost:8090/_/
2. Заполните форму создания админа:
   - Email: `admin@konvert.chat`
   - Password: `ваш_безопасный_пароль`

### Проверьте работу:

```bash
# Тест подключений
cd backend-setup
node test-connection.js

# Должно показать:
# ✓ PocketBase: 5/5 тестов пройдено
# ✓ Redis: 5/5 тестов пройдено
```

### Запустите приложение:

```bash
cd /path/to/konvert-chat
npm run dev
```

Откройте: http://localhost:3000

---

## 📊 Статус сервисов

### Проверка:

```bash
# PocketBase
sudo systemctl status konvert-pocketbase

# Redis
sudo systemctl status redis-server
```

### Управление:

```bash
# Перезапуск
sudo systemctl restart konvert-pocketbase
sudo systemctl restart redis-server

# Логи
sudo journalctl -u konvert-pocketbase -f
```

---

## 🔐 Безопасность

### Сгенерированные ключи:

Скрипт автоматически создает в `.env`:

- `JWT_SECRET` - для JWT токенов
- `ENCRYPTION_KEY` - для E2EE шифрования

**Сохраните эти ключи!** Они нужны для восстановления.

### Рекомендации для продакшена:

1. **Настройте firewall:**
```bash
sudo ufw allow 8090/tcp  # PocketBase
sudo ufw allow 6379/tcp  # Redis (только для localhost)
sudo ufw enable
```

2. **Настройте Nginx reverse proxy** (см. backend-setup/README.md)

3. **Установите SSL сертификат:**
```bash
sudo certbot --nginx -d your-domain.com
```

---

## 🧪 Тестирование

### Ручной тест:

```bash
# PocketBase health
curl http://localhost:8090/api/health

# Redis ping
redis-cli ping

# Статистика Redis
redis-cli INFO stats
```

### Автоматический тест:

```bash
cd backend-setup
node test-connection.js
```

---

## 🐛 Troubleshooting

### PocketBase не запускается:

```bash
# Проверка порта
sudo lsof -i :8090

# Убить процесс на порту 8090
sudo kill -9 $(sudo lsof -t -i:8090)

# Запуск вручную
cd /opt/pocketbase
./pocketbase serve --http=0.0.0.0:8090
```

### Redis не подключается:

```bash
# Проверка
sudo systemctl status redis-server

# Перезапуск
sudo systemctl restart redis-server

# Тест
redis-cli -h localhost -p 6379 ping
```

### Коллекции не создаются:

```bash
# Проверьте доступность PocketBase
curl http://localhost:8090/api/health

# Попробуйте создать вручную
cd backend-setup
node create-collections.js

# Или через Admin UI
# http://localhost:8090/_/
```

---

## 📚 Дополнительная информация

- **Полная документация:** [backend-setup/README.md](backend-setup/README.md)
- **Redis Cache API:** [backend-setup/redis-cache.js](backend-setup/redis-cache.js)
- **Миграция из Docker:** Не требуется, это нативная установка

---

## ✅ Чеклист

- [ ] PocketBase установлен
- [ ] Redis установлен
- [ ] Node.js 18+ установлен
- [ ] Запущен `./setup.sh`
- [ ] Создан администратор в PocketBase Admin UI
- [ ] Тест подключения пройден (`node test-connection.js`)
- [ ] Приложение запускается (`npm run dev`)

---

## 🎉 Готово!

После выполнения всех шагов у вас будет:

✅ PocketBase работает как системный сервис  
✅ Redis настроен для кэширования  
✅ 7 коллекций созданы  
✅ 8 достижений добавлены  
✅ E2EE шифрование готово  
✅ Real-time подписки работают  
✅ Backend полностью готов!  

**Запустите frontend и наслаждайтесь чатом! 🚀**

---

**Версия:** 1.0.0  
**Платформа:** Ubuntu 20.04+ (нативная установка)  
**Docker:** Не требуется ❌  
**Время установки:** ~5 минут ⚡
